...
class ResponseScorer:
    ...
    def score_response_quality_batch(self, requests: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            'avg_quality_score': sum(m['overall_score'] for m in quality_metrics) / len(quality_metrics),
            'avg_relevance': sum(m['relevance'] for m in quality_metrics) / len(quality_metrics),
            'avg_coherence': sum(m['coherence'] for m in quality_metrics) / len(quality_metrics),
            'avg_memory_utilization': sum(m['memory_utilization'] for m in quality_metrics) / len(quality_metrics),
