...
class TokenAnalyzer:
    ...
    def calculate_efficiency_metrics_batch(self, requests: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            'avg_efficiency_score': sum(m['efficiency_score'] for m in efficiency_metrics) / len(efficiency_metrics),
            'avg_net_savings': sum(m['net_savings'] for m in efficiency_metrics) / len(efficiency_metrics),
            'total_memory_tokens': sum(m['memory_tokens'] for m in efficiency_metrics),
            'total_actual_tokens': sum(m['actual_tokens'] for m in efficiency_metrics),
