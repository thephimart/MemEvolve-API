...
class ResponseQualityScorer:
    ...
    def calculate_response_quality(
        ...
        try:
            ...
            if self.debug:
                logger.info(f"Scoring response: content_length={len(assistant_content)}, "
                           f"reasoning_length={len(reasoning_content)}, has_reasoning={has_reasoning}")
            ...
            if has_reasoning:
                ...
                final_quality = self._evaluate_direct_response(assistant_content, base_quality, context, query)
    ...
    def _evaluate_reasoning_response(
        ...
        consistency_score = self._evaluate_reasoning_answer_consistency(reasoning_content, answer_content)
    ...
    def _check_query_analysis(self, reasoning: str, query: str) -> float:
        ...
        analysis_bonus = 0.2 if any(indicator in reasoning.lower() for indicator in analysis_indicators) else 0.0
    ...
    def _has_problem_breakdown(self, content: str) -> bool:
        ...
        breakdown = ['break down', 'first we need', 'let me consider', 'analyze the', 'identify the']
