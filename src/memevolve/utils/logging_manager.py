...
Provides directory-tree-based logging that mirrors the source code structure:
...
class LoggingManager:
    ...
    def _ensure_configured(cls):
        ...
        config = load_config()
        cls._log_dir = getattr(config.logging, 'log_dir', './logs') if hasattr(config, 'logging') else './logs'
        cls._base_level = getattr(config.logging, 'level', 'INFO') if hasattr(config, 'logging') else 'INFO'
    ...
    def _get_log_file_path(cls, logger_name: str) -> str:
        ...
        for part in path_parts[:-1]:  # All parts except the last (module name)
    ...
    @classmethod
    def get_logger(cls, name: str, level: Optional[str] = None, create_file: bool = True) -> logging.Logger:
        """
